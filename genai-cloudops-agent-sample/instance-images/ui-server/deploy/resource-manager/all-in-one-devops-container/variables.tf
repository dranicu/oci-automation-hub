...
variable "notification_topic_id" {
  description = "Existing ONS topic OCID for OCI DevOps project notifications."
  type        = string
}

variable "create_hosted_source_repository" {
  description = "Create an OCI DevOps hosted code repository and push bundled source into it."
  type        = bool
  default     = true
}

variable "hosted_source_repository_name" {
  description = "Name for the OCI DevOps hosted source repository."
  type        = string
  default     = "oci-agent-source"
}

variable "git_username" {
  description = "Git username for pushing to the OCI DevOps hosted repository."
  type        = string
  default     = ""
}

variable "git_auth_token" {
  description = "Git auth token for pushing to the OCI DevOps hosted repository."
  type        = string
  default     = ""
  sensitive   = true
}
...
variable "source_connection_type" {
  description = "Source repository connection type for OCI DevOps Managed Build."
  type        = string
  default     = "GITHUB"
}

variable "source_connection_id" {
  description = "OCI DevOps connection OCID for GitHub/GitLab/Bitbucket/VBS. Leave empty when source_connection_type is DEVOPS_CODE_REPOSITORY."
  type        = string
  default     = ""
}

variable "source_repository_url" {
  description = "Source repository URL for the application source."
  type        = string
  default     = ""
}
...
variable "source_repository_id" {
  description = "OCI DevOps Code Repository OCID. Required only when source_connection_type is DEVOPS_CODE_REPOSITORY."
  type        = string
  default     = ""
}

variable "build_spec_file" {
  description = "Build spec file path in the source repository."
  type        = string
  default     = "build_spec.yaml"
}

variable "build_runner_type" {
  description = "OCI DevOps build runner type."
  type        = string
  default     = "DEFAULT"
}

variable "build_image" {
  description = "OCI DevOps build environment image."
  type        = string
  default     = "OL7_X86_64_STANDARD_10"
}

variable "run_build" {
  description = "Start a build run from Resource Manager apply."
  type        = bool
  default     = true
}
