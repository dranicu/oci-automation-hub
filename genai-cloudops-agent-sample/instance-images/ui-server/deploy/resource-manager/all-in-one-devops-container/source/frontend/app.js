...
async function loadModels(region, projectId = projectSelect.value) {
  ...
  try {
    const res = await apiFetch(`/api/oci/models?region=${encodeURIComponent(region)}&project_id=${encodeURIComponent(projectId)}`);
    const data = await res.json();
...
function renderRagSource(source) {
  ...
  ragStatus.textContent = ragSource
    ? `${enabled ? "Enabled" : "Ready"} · ${(ragSource.file_ids || []).length} file(s)`
    : "Not configured";
  ...
  ragSourceOutput.textContent = `Vector store: ${ragSource.vector_store_id}\nUpdated: ${ragSource.updated_at || "unknown"}`;
}
...
async function uploadRagFiles() {
  ...
  try {
    ...
    appendTrace(`Uploaded ${uploadedIds.length} RAG file(s).`);
    await saveRagSource();
  } catch (err) {
...
projectSelect.addEventListener("change", async () => {
  saveSettings();
  await loadModels(regionSelect.value, projectSelect.value);
  await loadRagSource();
  updateSendState();
});
