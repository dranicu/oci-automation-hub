...
resource "oci_core_instance" "this" {
  ...
  fault_domain                        = each.value.fd != null ? format("FAULT-DOMAIN-%s", each.value.fd) : null
  ...
  metadata = {
    ...
    user_data = base64encode(templatefile("${path.module}/../../userdata/cloudinit.sh.tftpl", {
      ...
      region         = var.region
      estargz_image  = var.estargz_image
      regular_image  = var.regular_image
      run_validation = tostring(var.run_validation)
    }))
  }
}
