...
resource "oci_containerengine_cluster" "litmus" {
...
resource "oci_containerengine_node_pool" "litmus-pool" {
  compartment_id     = var.compartment_ocid
  cluster_id         = oci_containerengine_cluster.litmus.id
  name               = "litmus-pool"
...
data "oci_containerengine_node_pool_option" "node_pool_options" {
  compartment_id     = var.compartment_ocid
  node_pool_option_id = oci_containerengine_cluster.litmus.id
}
...
data "oci_containerengine_cluster_kube_config" "litmus" {
  cluster_id = oci_containerengine_cluster.litmus.id
}

provider "kubernetes" {
  host                   = yamldecode(data.oci_containerengine_cluster_kube_config.litmus.content).clusters[0].cluster.server
  cluster_ca_certificate = base64decode(yamldecode(data.oci_containerengine_cluster_kube_config.litmus.content).clusters[0].cluster["certificate-authority-data"])
  ...
  exec {
    ...
    args = [
      ...
      "generate-token",
      "--cluster-id",
      oci_containerengine_cluster.litmus.id,
    ]
  }
}
...
resource "kubernetes_namespace_v1" "litmus" {
  ...
  depends_on = [oci_containerengine_node_pool.litmus-pool]
}
...
resource "helm_release" "litmus" {
  ...
  # version          = var.litmus_chart_version

  values = [yamlencode({
    portal = {
      frontend = {
        service = {
          type = var.litmus_service_type
        }
      }
    }
  })]

  depends_on = [kubernetes_namespace_v1.litmus]
}

data "kubernetes_service_v1" "litmus_frontend" {
  metadata {
    name      = var.litmus_frontend_service_name
    namespace = kubernetes_namespace_v1.litmus.metadata[0].name
  }
